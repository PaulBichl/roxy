...
def _normalized_label(label) -> str:
...
def handle_detection(
    ...
    if normalized_label in roxy.IGNORED_CLASSES:
        roxy.lock()
    ...
    if normalized_label in roxy.PREY_CLASSES:
        roxy.lock()
        roxy.prey_latched = True
        logging.info("Flap locked due to prey class detected")
        return

    if normalized_label in roxy.SAFE_CLASSES:
